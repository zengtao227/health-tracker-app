...
### 9.1 黄历（黄历）计算：专业库驱动
**原则**: 使用行业标准的专业历法库，严禁硬编码年份数据。

*   **技术方案 (2026-02 升级)**:
    1.  **核心库**: `cn.6tail:lunar:1.7.7` (MIT License, Kotlin/Java)
    2.  **覆盖范围**: 1900-2100 年精确农历数据，自动处理闰月、大小月、立春分界
    3.  **API 使用**:
        ```kotlin
        val solar = Solar.fromDate(date)
        val lunar = solar.lunar
        lunar.yearInGanZhi      // 干支年 (如 "乙巳")
        lunar.yearShengXiao     // 生肖 (如 "蛇")
        lunar.monthInChinese    // 农历月 (如 "正月")
        lunar.dayInChinese      // 农历日 (如 "初一")
        lunar.zhiXing           // 建除十二神 (如 "建")
        lunar.dayYi             // 宜 (List<String>)
        lunar.dayJi             // 忌 (List<String>)
        ```
    4.  **废弃**: 所有硬编码的月份数组、年份常量已删除
