...
### 9.1 黄历（黄利）计算：建除十二神逻辑
**原则**: 严禁使用随机语料，必须基于“天文偏移”算法推演。

*   **计算模型**:
    1.  **基准锚点**: 以 `2024-02-10` (农历甲辰年正月初一，地支为“甲辰”) 为日期偏移基准 `refDate`。
    2.  **日地支推算**: `branchIndex = (4 + diffDays % 12 + 12) % 12`。
    3.  **月地支推算**: `monthBranchIndex = (currentMonth + 2) % 12`。
    4.  **神位判定**: `shenIndex = (branchIndex - monthBranchIndex + 12) % 12`。
*   **神位映射**: 结果对应“建、除、满、平、定、执、破、危、成、收、开、闭”十二神。
*   **宜/忌生成**: 根据当前神位动态映射对应的宜忌事项（而非 Math.random）。

### 9.2 星座（新作）判定：公历区间法
**原则**: 废弃字符串哈希或随机分配，严格遵循太阳黄道星座日期。
...
*   **判定标准**:
    ...
    - ... (依此类推)
*   **逻辑实现**: 必须通过传入的具体日期（月、日）进行判定。

### 9.3 UI 呈现最佳实践：防碎词技术 (Anti-breaking)
**原则**: 增强专业感，防止中文术语（如“裁衣”、“纳采”）在行尾被断开。

*   **实现方法**: 在词语内部的每个汉字之间插入 **Word Joiner (`\u2060`)** 不可见字符。
*   **渲染效果**: 词语内部禁止断行，词语之间（由普通空格分隔）允许断行，确保视觉排版整齐。
